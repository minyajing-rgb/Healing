"""Exercise the enhanced map without replacing existing story/time acceptance checks."""
...
checks=[];errors=[];bad=[];detail={}
def check(name,value):
    checks.append({'check':name,'passed':bool(value)});assert value,name
try:
 with sync_playwright() as p:
  browser=p.chromium.launch(args=['--enable-unsafe-swiftshader','--use-angle=swiftshader','--disable-web-security'])
  ...
  state=page.evaluate('window.EARTH_HEALING_MAP.state()')
  check('MapLibre engine initialized',page.evaluate('window.EARTH_HEALING_MAP.engine')=='maplibre')
  check('GL map is visible',page.locator('#mapGl').is_visible())
  check('Pins use the same published data as story rail',state['visible']==page.evaluate('window.EARTH_HEALING.state().visible'))
  check('Nearby places are clustered',page.locator('.is-cluster').count()>0)
  check('Interactive map markers exist',page.locator('.eh-map-pin').count()>0)
  ...
  page.wait_for_function('window.EARTH_HEALING_MAP.state().visible===8')
  check('1750 slider drives new map',page.evaluate('window.EARTH_HEALING_MAP.state().visible')==8)
  page.locator('#clearFilters').click();page.locator('#regionFilter').select_option('europe')
  page.wait_for_function('window.EARTH_HEALING_MAP.state().zoom>2.5')
  check('Region selector moves camera',page.evaluate('window.EARTH_HEALING_MAP.state().zoom')>2.5)
  ...
  page.wait_for_function('window.EARTH_HEALING_MAP.state().visible===1')
  page.locator('.eh-map-pin[data-story]').first.click()
  check('New map pin opens actual sourced story',page.locator('#storyDialog').evaluate('e=>e.open') and page.locator('.source-list a').count()>0)
  page.keyboard.press('Escape');page.locator('#clearFilters').click()
  before=page.evaluate('window.EARTH_HEALING_MAP.state().zoom');page.locator('#zoomIn').click()
  page.wait_for_function('window.EARTH_HEALING_MAP.state().zoom>'+str(before))
  ...
  check('Map controls translated','Garden atlas' in page.locator('.map-style-switch').inner_text())
  page.locator('#regionFilter').select_option('europe');page.wait_for_timeout(300)
  page.locator('[data-map-style="detail"]').click();page.wait_for_timeout(12000)
  detail={'state':page.evaluate('window.EARTH_HEALING_MAP.state()'),'hint':page.locator('#mapExperienceHint').inner_text()}
  check('Optional external style has a safe usable outcome',detail['state']['style'] in ['garden','detail'])
  ...
  check('Switch back to self-hosted garden style',page.evaluate('window.EARTH_HEALING_MAP.state().style')=='garden')
  ...
  check('Mobile enhanced engine works',mobile.evaluate('window.EARTH_HEALING_MAP.engine')=='maplibre')
  ...
  mobile.locator('#fontToggle').click();check('Large text remains in viewport',mobile.evaluate('document.documentElement.scrollWidth<=innerWidth+2'))
  fallback=browser.new_page(viewport={'width':1000,'height':800})
  fallback.add_init_script("Object.defineProperty(window,'maplibregl',{get:()=>undefined,set:()=>{}})")
  ...
  check('Unsupported engine falls back safely',fallback.evaluate('window.EARTH_HEALING_MAP.engine')=='svg-fallback')
  fallback.locator('#listView').click();check('Fallback exposes readable story list',fallback.locator('#mapList').is_visible())
  check('No new JavaScript errors',not errors);check('No missing local resources',not bad)
  browser.close()
except Exception:
 (REPORT/'map-v2-failure.txt').write_text(traceback.format_exc());raise
finally:
 server.shutdown();report={'checks':checks,'errors':errors,'http_failures':bad,'detail_service':detail}
