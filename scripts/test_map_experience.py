...
try:
 with sync_playwright() as p:
  ...
  check('Leaflet cloud detail activates without API key',page.evaluate("window.EARTH_HEALING_MAP.state().cloud?.engine==='leaflet' && window.EARTH_HEALING_MAP.state().cloud?.provider==='OpenStreetMap'"))
  check('Cloud map has OpenStreetMap tiles',page.locator('#cloudMapPane .leaflet-tile').count()>0)
  page.locator('.explorer-shell').screenshot(path=str(REPORT/'map-cloud-detail.png'))
