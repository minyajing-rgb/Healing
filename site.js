...
(() => {
  ...
  const art=(s,label=false)=>'<div class="art" data-art="'+escapeHTML(s.art||s.themes?.[0]||'plant')+'">'+icon(s.art==='garden'?'aroma':s.art)+(label?'<span class="art-label">'+tx('概念插画','CONCEPT ILLUSTRATION')+'</span>':'')+'</div>';
  ...
  function renderFilms(){
    ...
    html+=media.map(m=>'<article class="film-card"><video controls playsinline preload="none" poster="./assets/garden.avif" aria-label="'+escapeHTML(m[lang]||m.en)+'"><source src="'+escapeHTML(m.path)+'" type="video/webm"></video><div class="film-copy"><h3>'+escapeHTML(m[lang]||m.en)+'</h3><p>'+tx('用户提供的概念影像 · 低清预览，不是历史证据','User-supplied concept film · low-resolution preview, not historical evidence')+'</p></div></article>').join('');
