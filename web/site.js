...
(() => {
  ...
  function renderFilms(){
    ...
    html+=media.map(m=>'<article class="film-card"><video controls playsinline preload="none" poster="./assets/garden.avif" aria-label="'+escapeHTML(m[lang]||m.en)+'"><source src="'+escapeHTML(m.path)+'" type="video/mp4"></video><div class="film-copy"><h3>'+escapeHTML(m[lang]||m.en)+'</h3><p>'+tx('用户提供的概念影像 · 低清预览，不是历史证据','User-supplied concept film · low-resolution preview, not historical evidence')+'</p></div></article>').join('');
